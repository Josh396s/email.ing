...
This file deals with JWT tokens. It takes user data and an expiration date, and then generates an encoded token that the user will use to access secure information in the application. There is also a function that decodes the the token and ensures that it is valid or else it sends an error. The final function uses the token to check to see if the user actually exists.
...
> A form of stateless token security. A timed token is generated for each user and is maintained by the user. When accessing secure information, the user will provide the token and the server authenticates it.
